#!/usr/bin/env python3
"""Role-explicit TCP teleoperation leader entrypoint."""
...
from lerobot_remote.runtime.teleoperation import run_tcp_teleop_leader_client
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Validate config and print resolved leader settings without touching hardware.",
    )
...
def main() -> int:
    ...
    print(json.dumps(configured_runtime_summary("tcp-teleop-leader", config), indent=2, sort_keys=True))
    if args.dry_run:
        return 0
